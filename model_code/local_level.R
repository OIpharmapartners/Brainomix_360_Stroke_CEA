...
# - inputs/IO_isdn_hosp_numbers.csv  # hospital numbers of stroke patients
#
# OUTPUTS:
# - outputs/hospital_results.RData
# - outputs/hospital_incremental_results.csv
#
# DEPENDENCIES:
...
hospital_results <- list() ## create to put outputs in

hospital_l[ , group_ID := .GRP, by =.(Hospital)]
...
for (i in 1:max(unique(hospital_l$group_ID))){
  ...
  # Ensure x has only one row
  assert_that(nrow(x) == 1, msg = paste("Group", i, "has", nrow(x), "rows — expected 1."))
  ...
  if (is_csc) {
    temp_data_main[model_param == "p.asc", base_case := 0]
    temp_data_main[model_param == "n.asc", base_case := 0]
    temp_data_main[model_param == "n.csc", base_case := 1]
  } else {
    temp_data_main[model_param == "p.asc", base_case := 1]
    temp_data_main[model_param == "n.asc", base_case := 1]
    temp_data_main[model_param == "n.csc", base_case := 0]
  }
  
  y <- run_model(temp_data_main, cycles = 10, mrs_samples_mean = mrs_samples_mean)
  
  hospital_results[[hospital]] <- y
}

save(hospital_results,file="outputs/hospital_results.RData")

## save table of incremental results

incremental_df <- do.call(rbind, lapply(names(hospital_results), function(region) {
  result <- hospital_results[[region]]$incremental_results
...
write.csv(incremental, file = "outputs/hospital_incremental_results.csv", row.names = FALSE)
